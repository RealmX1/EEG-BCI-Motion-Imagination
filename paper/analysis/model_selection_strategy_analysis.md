...
- 训练参数：使用 cross-subject HPO 优化的参数（`get_cross_subject_config()`），该 HPO 同样使用 `combined_score` 作为目标函数
- 128ch, binary classification, CBraMod
- 每个策略独立 run，seed=42

### 6.2 HPO 偏置声明（扩展）

与 within-subject 相同，cross-subject 的 HPO 也使用 `combined_score` 作为目标函数，因此超参数天然偏向 combined 策略。其他策略（val_acc、ema、soup）使用的是为 combined 优化的参数，可能处于不公平的劣势。受算力限制，本实验不对每个策略单独进行 HPO 调优。
...
三个范式的 HPO 目标函数均使用 `combined_score`（within-subject HPO Trial #46, cross-subject HPO），因此超参数天然偏向 combined 策略。其他策略使用为 combined 优化的参数，可能处于不公平的劣势。受算力限制，本实验不对每个策略单独进行 HPO 调优，结论性判断需要在各策略各自完成 HPO 后才能做出。EMA 尤其需要针对性地调整 decay 参数和更新频率。
