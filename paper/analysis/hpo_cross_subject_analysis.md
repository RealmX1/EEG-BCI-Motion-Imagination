...
### Top 10 vs Bottom 10 效应量分析

由于完成 trial 较多 (n=43)，使用 Top 10 vs Bottom 10 的 Cohen's d 效应量代替 fANOVA：
...
**1. 正则化参数接近默认即可，微调空间有限**

与 within-subject HPO 的"大幅降低正则化"结论不同，cross-subject 的最优 dropout (0.37) 和 weight_decay (0.13) 与默认值 (0.35, 0.12) 几乎相同。跨被试数据分布更复杂，确实需要较强的正则化。但从效应量看，weight_decay 和 label_smoothing 的 Bottom 10 偏高（过度正则仍有害），最优区域在默认值附近。

**2. gradient_clip 放宽是最大杠杆之一**

gradient_clip 从默认 0.5 提升至 1.36 (Top 10 均值 1.22)，效应量 +0.82。默认值 0.5 过于保守，过度裁剪梯度限制了模型学习能力。

**3. CAWD scheduler: 长 phase + 短探索 + 大探索 batch**

- phase_epochs: 6→10（每阶段训练更充分）
- exploration_epochs: 6→3（缩短小 batch 探索阶段）
- exploration_batch_size: 32→128（探索阶段也用大 batch）

这与 within-subject HPO 的发现高度一致。跨被试数据量大，不需要长时间的小 batch 探索。

**4. 学习率改动幅度很小**

backbone_lr (1e-4 → 1.34e-4) 和 classifier_lr_ratio 变化很小，效应量仅 +0.25 和 +0.61。默认学习率设置对 cross-subject 已较为合理。

**5. label_smoothing 效果反转**

HPO 最优 label_smoothing=0.285 高于默认 0.15，但效应量分析显示"低更好"(d=−0.77)。这说明 Trial #4 的高 label_smoothing 可能是该 trial 其他参数组合的偶然产物，不应盲目追随。综合看 0.10–0.20 是较稳健的区间。
...
| 核心调优方向 | 降低正则 + 升高 LR | 放宽 gradient_clip + 优化 scheduler |
