...
为评估通道缩减对模型性能的影响，我们沿 **128 → 64 → 61 → 32 → 16 → 8 → 4ch** 七档通道数 × 5 种通道选择方法 (FDR / Band Power / Attention / CSP / negative_control) × binary / ternary 双 task 完成跨被试 CBraMod 全矩阵（每 cell 1 次 21-被试训练，无 replicate）。Binary baseline 取 128ch CBraMod cross-subject `20260324_0023`（mean **90.68%**），ternary baseline 取 `20260324_0109`（mean **74.88%**，run-to-run range 73.06–75.50% 跨 6 个完整 21 人 run，详见 §3.2）。图 4d 把全部 cell 整合为 "% retention vs 128ch baseline" 双面板图，每条曲线带 **±1 subject-std / baseline** 阴影 —— 单 cell Δ 小于带宽时不具备方法学解读价值。**本节因此只做结构性陈述**，不对单 cell 数值做深入解读；方法 / 解剖机制讨论集中在 §3.5.3。

**图 4d. 通道缩放 % retention（cross-subject CBraMod, N = 21；binary | ternary 双面板，shared y）。** 横轴对数刻度 [4, 8, 16, 32, 61, 64, 128]；纵轴 % retention = N-ch_acc / 128ch_baseline_acc × 100。彩色实线 + 同色 ±1 subject-std/baseline 阴影 = 5 method（FDR 红 / Band Power 绿 / Attention 蓝 / CSP 橙 / negative_control 灰）；hollow 黑圆 = 128ch baseline anchor（100% retention）；hollow 紫方 = 61ch standard 10-10 single-config（不属方法族）；橙菱 = 4ch FDR ∩ Att 交集（binary only, outlier）；红虚线 = chance retention（binary ≈ 55% / ternary ≈ 45%）。**核心阅读规则**：当任意两 method 曲线差落入彼此 std 阴影带内时应视为不可区分。

![图 4d. 通道 % retention faceted（binary | ternary）](../figures/channel_retention_faceted.png)

**表 9b. 通道缩放最优 envelope 摘要（每档 best-method retention）。** Best = 该通道档 5 method 中绝对准确率最高者；retention = best_acc / 128ch_baseline × 100。完整 5 × 5 × 2 矩阵数值通过 `paper/run_registry.yaml` 的 `reduced_{N}_{method}_{task}` alias 可全部检索（见下方数据来源行），本表仅列 envelope。**与 v3.1 早期版本的差别**：原 "Table 9" 为 24 行 method × channel × task 全 cell δ pp 表（dual baseline 约定），现已 retire；其完整信息整合到图 4d 与本 envelope 摘要表中，避免对单 run cell 数值过度解读（每 cell = 1 次 21-被试训练，无 replicate）。引用旧 "Table 9 row X" 的外部笔记应改引图 4d / `run_registry.yaml` alias。

| 通道档 | Binary best (acc) | Binary retention | Ternary best (acc) | Ternary retention |
|---|---|---:|---|---:|
| 64 | FDR (89.46%) | 98.7% | FDR (75.12%) | 100.3% |
| 32 | FDR (87.71%) | 96.7% | BP (72.20%) | 96.4% |
| 16 | BP (85.24%) | 94.0% | FDR (69.31%) | 92.6% |
...
**图 3d. Reduced-channel × method × task 矩阵全景（cross-subject CBraMod, N = 21；2 × 6 grid, 12 panel）。** Row：binary（panel A-F）/ ternary（panel G-L）；col：4 / 8 / 16 / 32 / 61 / 64 ch。每 panel 5 method 分组柱（FDR / Att / BP / CSP / NegCtrl），4ch 列额外含 FDR ∩ Att 交集柱，61ch 列为 standard 10-10 单配置；柱高 = mean cross-subject accuracy，error bar = subject-std。横虚线 = 128ch baseline（binary 90.68% / ternary 74.88%）；右上 bracket = top bar 与 baseline 的 Δ。

![图 3d. reduced-channel × method × task 矩阵（4/8/16/32/61/64ch, 2×6 grid）](../figures/reduced_channel_40cell_grid.png)

**图 4. 通道缩放曲线（cross-subject CBraMod binary | ternary 双面板）。** 红色实线 = 每通道档最优方法的绝对准确率包络（6 档降解：128 → 64 → 32 → 16 → 8 → 4）；虚线追踪 per-method；紫菱 = 61ch standard 10-10 single-config outlier；橙星 = 4ch FDR ∩ Att overlap outlier；× = negative_control；绿色阴影 = 32ch 部署区间；红虚线 = chance level。

![图 4. 通道缩放曲线（128 → 4 envelope）](../figures/channel_scaling_curve.png)

**图 4b. 64 / 32 / 16 / 8 / 4ch 通道选择方法排序翻转 slope chart（binary | ternary 双面板）。** 4 数据驱动方法（FDR / Band Power / CSP / Attention）在 5 档通道数下的 cross-subject CBraMod 准确率；每档列名给出当前 ranking。Binary：32ch+ FDR 领先，16ch 起 BP 反超并在 8 / 4ch 单独保持；ternary：BP 在 32ch 已超 FDR，16ch 起 BP / FDR 在前两位互换。4ch negative_control 虚线作天花板对照。
...
> 生成命令: 图 4d 由 `uv run python scripts/paper/generate_paper_figures.py --figure fig4d` 生成；图 4 由 `--figure channel_scaling`；图 4b 由 `--figure channel_ranking_flip`；图 3d 由 `--figure reduced_channel_40cell_grid`（2 × 6 grid, 4/8/16/32/61/64ch）。所有 5 个图的版本链 + staging 通过 `/figure-snapshot-diff` 工作流管理：`uv run python .claude/skills/figure-snapshot-diff/scripts/history_server.py --port 8765` 浏览 trunk / staging / 评论。
...
> **重要说明**：FDR∩Attention 的 4 个通道（B32, C8, D7, D19；空间位置见 Figure S6b）并非任何单一方法排序的 top-4，而是两个 32 通道集合的相对小交集（128 通道全空间中 32×32/128 = 8 的随机交集期望，本研究实际交集为 4）——它们在各自单方法排序中仅位于第 15–30 位。82.71% 的高准确率应被视为一个**有利的巧合**（favorable outlier）：这是从相对小的交集中"碰巧"落到的 4 个位置，并非"被两种方法共同认定为最重要"的强一致选择，因而**不能作为系统化方法复制**——详细抽样机制讨论见下方 "FDR∩Att outlier 抽样机制" 段。**Band Power top-4** 与 outlier 不同，它是单一方法 top-4 的标准化输出（mu/beta 频带 ANOVA F 统计量前 4 通道：**B28, C23, D11, D27**，详见 `results/4_channel/channel_selections.json`；解剖位置详见下方 "BP top-4 解剖学定位" 段），是可复现的系统化选取——下方"4 通道是否可用"的部署判断主要依赖 Band Power 这一可复现路径，而非 FDR∩Attention 这一 outlier。
...
**BP top-4 解剖学定位**——Band Power top-4 选出的 4 个通道在 BioSemi 128 layout 中的位置经在线对照官方 `Cap_coords_all.xls` 后整理如下：
...
| **16ch** | **8.69 pp** | **7.64 pp** | BP 85.24 → Attention 76.55 | FDR 69.31 → Attention 61.67 |
...
> 注：方法间 spread 在 binary / ternary 上**双 task 同向单调扩张**——在本数据集与本任务的五档观察上，通道选择方法间的差异随通道数减少而扩大；32ch+ 上 spread ≤ 3.24 pp（落在被试间 std ≈ 13 pp 的 noise 量级），16ch 上跃升 3–4 倍到 8.69 / 7.64 pp（**method-agnostic 区间的崩溃入口**），低通道档 spread 急剧扩至 4ch binary 24 pp。32ch 的 binary 最优方法（FDR）在 8ch / 4ch / 32ch ternary / 64ch ternary 上均不再领先——具体的 leading method 在不同 (通道, task) 组合上不同（binary 64/32ch: FDR；binary 16ch: BP（接近 FDR）；binary 8/4ch: BP；ternary 16ch: FDR；ternary 32/8/4ch: BP；ternary 64ch: FDR 但与 neg_ctrl indistinguishable）。我们不把"最优方法在通道数变化下重排序"概括为通用方法论命题，仅作为本研究中的具体观察。**Band Power 在 10 个 cell（5 档 × 2 task）上从未是 4 数据驱动方法的最差者**，是本数据集下最稳健的横向观察。4ch FDR∩Attention 交集 (82.71%) 为 favorable outlier，非标准单方法选择，不纳入方法间差异计算；ternary 上 FDR∩Attention 未跑（仅 binary `20260330_1417` 存在）。
...
**图 4c. 通道选择方法敏感度随通道数缩放。** 左 y 轴（红色）：四种方法在该档位的 max−min 准确率 spread；右 y 轴（蓝色）：每档位最优方法的绝对 cross-subject 准确率（按 §3.5.2 envelope 表 9b / §3.5.3 表 10 的最高值）。Binary | ternary 双面板，x 轴对数刻度 [4, 8, 16, 32, 64]。
