...
在纵向数据扩展方面，对 16 名拥有 3–5 个额外在线 session 的被试进行分析表明，增量训练数据可显著突破性能平台期：二分类中 EEGNet 从 80.51% 提升至 87.85%（+7.34 pp，p = 0.007），CBraMod 从 87.23% 提升至 92.93%（+5.70 pp，p = 0.015），被试间标准差从 10.46% 压缩至 5.92%。在三分类中，CBraMod 同样显著获益（+8.55 pp，p = 0.012）。迁移学习在二分类和三分类上均未突破跨被试基线（Δ 分别为 −0.56 pp 和 +0.20 pp）。
...
>
> 4. **多 session 纵向数据扩展**。首次在手指级 MI 分类任务上证明基座模型和轻量 CNN 均从额外 session 数据中显著获益（CBraMod: +6.13 pp 至 93.36%；EEGNet: +7.34 pp 至 87.85%，N = 16），低基线被试获益最大（+15 pp），被试间方差压缩近半。
>
...
每一步训练集扩大，测试集为最新 session 的 Finetune 部分。补充分析中使用 fixed_combined（固定组合测试集）和 fixed_sess02（固定 Sess02 测试集）两种策略控制测试集难度变化的混淆因素，详见 Supplementary。
...
| +Sess03 | 89.65 ± 7.09% | +2.42 pp | — |
| +Sess04 | 91.84 ± 6.91% | +4.61 pp | — |
| +Sess05 | **92.93 ± 6.11%** | **+5.70 pp** | **0.015** |
...
| +Sess03 | 87.50 ± 6.76% | +6.99 pp | — |
| +Sess04 | 89.06 ± 8.49% | +8.55 pp | — |
| +Sess05 | **87.85 ± 6.82%** | **+7.34 pp** | **0.007** |

两种模型均从额外 session 数据中显著获益（p < 0.05）。CBraMod 从 87.23% 提升至 92.93%（+5.70 pp），EEGNet 从 80.51% 提升至 87.85%（+7.34 pp），EEGNet 的绝对增益更大但起点更低。图 7 展示了逐被试的渐进式性能变化。
...
|---------|---------------------|----------|-----------|
| 低基线 (<80%) | 8 / 3 | **+13.12 pp** | **+18.54 pp** |
| 高基线 (>90%) | 5 / 9 | −1.12 pp | +0.35 pp |

> 注：分组阈值 80%/90% 为绝对值，因两模型基线分布不同，各分组样本量有差异。CBraMod 低基线仅含 3 名被试（S06, S10, S16），+18.54 pp 的增益估计受个体差异影响大，应视为方向性趋势而非精确效应量。

低基线被试是额外 session 数据的主要受益者，高基线被试显示天花板效应。

**标准差压缩与观测范围**：CBraMod 的被试间标准差从 10.81% 压缩至 6.11%（−43%），表明额外数据不仅提高平均性能，还改善了跨用户一致性。实际观测范围从 60.62%–99.38%（Baseline）收窄至 71.88%–98.12%（+Sess05），最低单被试准确率从 60.62%（S10）提升至 71.88%（S10），反映了底部用户的显著改善。这一压缩对临床部署尤为重要：BCI 系统的实用化要求不是"最好情况下多好"，而是"最差情况下够不够用"。

从机制层面看，低基线被试获益显著而高基线被试接近天花板，表明额外 session 数据的主要作用是**弥补个体训练数据不足**而非提升模型容量。这与 Section 3.2 的发现形成有趣的对照：EEGNet 未从*其他被试*数据的池化中显著获益（跨被试 −1.43 pp, p = 0.456），但能从*同一被试*的额外 session 中显著获益（+7.34 pp, p = 0.007），提示其瓶颈在于被试间特征异质性而非绝对数据量。

> **数据来源**: EEGNet baseline: `results/20260316_1411_comparison_cache_imagery_binary.json`; CBraMod baseline: `results/20260323_2237_comparison_cache_imagery_binary.json`; Extra sessions: `results/20260329_1357_extra_sessions_cache_imagery_binary.json`
...
CBraMod 三分类增益（+8.55 pp，p = 0.012）达到显著水平，而 EEGNet 增益虽为正（+4.60 pp）但未达显著（p = 0.166）。与二分类相比，三分类呈现两个特征：（1）CBraMod 的增益反而更大（+8.55 pp vs +5.70 pp），可能因为三分类基线较低（74.51% vs 87.23%），天花板效应更弱；（2）EEGNet 未能达到统计显著，反映其在多类任务上容量有限，额外数据的边际收益低于二分类。图 8 展示了三分类的渐进式变化。

**图 8. Extra Sessions 三分类渐进式性能变化（N = 16）。**

![图 8. Extra Sessions 三分类](../../results/20260331_0827_extra_sessions_imagery_ternary.png)
...
|------|----------|-----------|------|
| per_session（默认） | +7.34 pp | +5.70 pp | 临床最相关 |
| fixed_combined | +9.96 pp | +8.44 pp | 控制测试集难度 |
| fixed_sess02 | +8.51 pp | +4.37 pp | 最保守估计 |

fixed_combined 策略显示单调递增趋势（消除了测试集难度变化的混淆因素）。fixed_sess02 下 CBraMod 的增益明显小于 EEGNet（+4.37 pp vs +8.51 pp），可能存在两种解释：（1）基座模型对跨 session 时间分布漂移更敏感；（2）天花板效应——CBraMod 的 fixed_sess02 baseline（87.23%）高于 EEGNet（80.51%），更高起点下增益空间本身受限。两种因素可能同时起作用，当前数据不足以区分。详细策略对比分析见 Supplementary。

> **数据来源**: `docs/dev_log/experiments/extra_sessions_strategy_comparison.md`

#### 3.5.4 三范式收敛

在充足 session 数据下，三种训练范式（被试内、跨被试、迁移学习）的性能趋于收敛：

**表 15. 三范式收敛（CBraMod 二分类，+Sess05，N = 16）。**

| 范式 | +Sess05 准确率 | Δ vs. Baseline |
|------|---------------|---------------|
| 被试内 | 92.93% | +5.70 pp |
| 跨被试 | 93.83% | +2.19 pp |
| 迁移学习 | **95.12%** | +3.28 pp |

三种范式在 +Sess05 时的准确率差距仅 ~2 pp（92.93%–95.12%），远小于 baseline 阶段的 ~5 pp 差距。被试内范式绝对增益最大（+5.70 pp），因其起点最低（87.23%）；跨被试和迁移学习范式起点已较高，增量空间有限。

> **数据来源**: 被试内: `results/20260329_1357_extra_sessions_cache_imagery_binary.json`; 跨被试: `results/20260329_2041_cross_subject_extra_sessions_cache_imagery_binary.json`

#### 3.5.5 跨被试范式下的额外 session 效果

被试内范式从额外 session 数据中显著获益（Section 3.5.1–3.5.2），但跨被试范式的响应模式截然不同。表 15b 展示了跨被试训练 + 额外 session 数据的二分类结果。

**表 15b-i. CBraMod 跨被试训练 + 额外 session 数据（二分类，N = 16）。**

| 阶段 | Mean ± SD | Δ vs Baseline |
|------|-----------|---------------|
| Baseline | 92.38 ± 8.08% | — |
| +Sess03 | 91.87 ± 6.50% | −0.51 pp |
| +Sess04 | 92.19 ± 6.69% | −0.19 pp |
| +Sess05 | **93.24 ± 5.63%** | **+0.86 pp** |

**表 15b-ii. EEGNet-16,4 跨被试训练 + 额外 session 数据（二分类，N = 16）。**

| 阶段 | Mean ± SD | Δ vs Baseline |
|------|-----------|---------------|
| Baseline | 81.45 ± 10.53% | — |
| +Sess03 | 81.84 ± 8.01% | +0.39 pp |
| +Sess04 | 82.54 ± 10.05% | +1.09 pp |
| +Sess05 | **81.33 ± 9.84%** | **−0.12 pp** |

EEGNet 跨被试范式下额外 session 数据**完全无效**（81.45% → 81.33%，Δ = −0.12 pp），与被试内范式 +7.34 pp 的显著增益形成鲜明对比。这一差异进一步印证了 EEGNet 的核心瓶颈：~10K 参数无法从多被试异质数据中提取共享表征。在跨被试框架下，即使新增数据来自同一被试的额外 session，这些数据仍被稀释于 21 名被试的联合训练池中，EEGNet 无法将其与对应被试关联。CBraMod 跨被试 baseline 已达 92.38%，额外 session 仅提供微弱改善（+0.86 pp），这与三范式收敛（表 15）的发现一致：跨被试模型在 baseline 阶段已接近天花板。
...
所有条件下 further pre-training 均导致性能下降或无改善。图 9 以柱状图直观展示了这一负面结果。

**图 9. Further Pre-training 下游评估。** 左图：四种条件下 Baseline (TUEG) vs FT-V1 vs FT-V2 的准确率对比，红色标注显示 V2 相对 Baseline 的变化量（均为负值）。右图：V1 和 V2 的平均 delta，V2 训练更充分但负迁移更大。

![图 9. Further Pre-training 下游评估](../../paper/figures/further_pretraining.png)
...
图 10 以对数坐标柱状图和模型规模对比直观呈现了这一结果。

**图 10. 推理延迟与模型规模对比。** 左图：不同 batch size 下两种模型的延迟（对数坐标），红色虚线为 100ms 实时阈值。右图：CBraMod/EEGNet 的参数量、FLOPs、模型大小、延迟倍率。

![图 10. 推理延迟对比](../../paper/figures/inference_latency.png)
...
原始数据集论文 [3] 在在线 base/fine-tuned EEGNet 设置下报告：被试性能在 2–3 个 session 后趋于平台期。本研究的 N = 16 离线分析回答的是一个更弱、也更可控的问题：如果去掉实时反馈与 same-day update，仅保留累积数据量增长，两种模型是否仍能从额外 session 中持续获益？结果显示答案是肯定的：在离线重训练范式下，EEGNet 与 CBraMod 均从额外 session 数据中显著获益（EEGNet p = 0.007, CBraMod p = 0.015），低基线被试尤为突出。但由于两项研究的训练闭环不同，本文结果不能简单推翻 [3] 的在线平台期结论，而应理解为揭示了“在线适应/反馈限制”与“纯数据量效应”之间的分离。

标准差从 10.85% 压缩至 5.89%（−46%）具有实际部署意义：BCI 系统需要跨用户的一致性能，而非少数用户的峰值表现。额外 session 数据不仅提高了平均水平，还将"最差情况"显著抬升。
...
>
> **发现 3 — 多 session 数据改善性能。** 额外 session 数据为两种模型均带来显著增益（CBraMod +5.70 pp, p = 0.015; EEGNet +7.34 pp, p = 0.007），低基线被试获益尤为突出，被试间标准差压缩约 43%（10.81% → 6.11%），最低单被试准确率从 60.62% 提升至 71.88%。
>
...
|------|---------|---------|---------|---------|-----------|
| S02 | 94.38% | 97.50% | 96.25% | 95.00% | +0.62 pp |
| S03 | 94.38% | 93.12% | 98.12% | 95.62% | +1.24 pp |
| S04 | 91.88% | 87.50% | 96.25% | 96.88% | +5.00 pp |
| S06 | 74.38% | 82.50% | 78.75% | 91.25% | +16.87 pp |
| S07 | 81.88% | 84.38% | 81.25% | 95.00% | +13.12 pp |
| S08 | 93.12% | 91.25% | 95.00% | 95.00% | +1.88 pp |
| S09 | 99.38% | 98.12% | 96.88% | 94.38% | −5.00 pp |
| S10 | 60.62% | 70.00% | 80.00% | 71.88% | +11.26 pp |
| S11 | 89.38% | 86.88% | 95.62% | 95.62% | +6.24 pp |
| S13 | 95.62% | 90.00% | 93.75% | 95.62% | +0.00 pp |
| S14 | 83.12% | 88.12% | 88.12% | 93.12% | +10.00 pp |
| S15 | 92.50% | 96.25% | 93.75% | 93.12% | +0.62 pp |
| S16 | 70.62% | 86.88% | 98.75% | 98.12% | +27.50 pp |
| S17 | 84.38% | 91.88% | 86.88% | 87.50% | +3.12 pp |
| S18 | 91.88% | 92.50% | 90.00% | 93.75% | +1.87 pp |
| S19 | 98.12% | 97.50% | 100.00% | 95.00% | −3.12 pp |

**EEGNet-16,4:**

| 被试 | Baseline | +Sess03 | +Sess04 | +Sess05 | Δ (总变化) |
|------|---------|---------|---------|---------|-----------|
| S02 | 94.38% | 92.50% | 95.62% | 93.12% | −1.26 pp |
| S03 | 85.00% | 82.50% | 91.25% | 88.12% | +3.12 pp |
| S04 | 94.38% | 93.12% | 92.50% | 90.62% | −3.76 pp |
| S06 | 68.12% | 85.00% | 73.12% | 91.25% | +23.13 pp |
| S07 | 76.88% | 85.00% | 83.12% | 89.38% | +12.50 pp |
| S08 | 85.00% | 92.50% | 93.12% | 95.62% | +10.62 pp |
| S09 | 99.38% | 96.25% | 97.50% | 95.62% | −3.76 pp |
| S10 | 70.00% | 75.62% | 77.50% | 76.25% | +6.25 pp |
| S11 | 70.00% | 83.75% | 88.12% | 90.00% | +20.00 pp |
| S13 | 91.88% | 88.75% | 94.38% | 92.50% | +0.62 pp |
| S14 | 78.12% | 81.25% | 82.50% | 78.12% | +0.00 pp |
| S15 | 71.25% | 90.62% | 89.38% | 84.38% | +13.13 pp |
| S16 | 56.25% | 81.88% | 96.25% | 83.12% | +26.87 pp |
| S17 | 70.62% | 78.12% | 74.38% | 73.75% | +3.13 pp |
| S18 | 91.25% | 96.88% | 96.25% | 93.75% | +2.50 pp |
| S19 | 85.62% | 96.25% | 100.00% | 90.00% | +4.38 pp |

> **数据来源**: `results/20260329_1357_extra_sessions_cache_imagery_binary.json`
...
> **数据来源**: per_session: `results/20260329_1357_extra_sessions_cache_imagery_binary.json`; fixed_combined: `results/20260325_0514_extra_sessions_cache_fixed_combined_imagery_binary.json`; fixed_sess02: `results/20260325_1208_extra_sessions_cache_fixed_sess02_imagery_binary.json`
