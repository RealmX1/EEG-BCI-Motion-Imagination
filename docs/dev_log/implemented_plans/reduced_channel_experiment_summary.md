...
<!-- 随后将通道选择基础设施**泛化至任意通道数**（`get_nch_indices` 通用函数、`channel_config`/`channel_n_target` 字段、training 脚本 strategy E 逻辑统一），使 `run_32ch_experiment.py` 支持 `--channels`/`--models`/`--steps`，`run_32ch_config_comparison.py` 支持 `--channels`。 -->
最后回到8ch执行了 **8ch FDR 实验**（binary + ternary cross-subject + transfer），发现 32→8ch 存在约 20% 的性能断崖，且 transfer 微调在低通道数下效果更显著（+4.59%/+5.26%）。所有实验结果已记录至 `docs/dev_log/implemented_plans/32ch_experiment.md`。
